...
//vetify customer when c login
router.post('/verify', function (req, res) {
    CustomerDao.cName2cId(req.body.name,function(cid){
        if(cid === undefined){
            res.status(400).json({result: "CustomerNotExist"});
            return
        }
        CustomerDao.findById(cid,function(cusotmer){
            if(cusotmer.password === req.body.password){
                res.status(200).json({result: "Valid",id: cusotmer.id});
                return
            }
            else{
...
router.delete('/:cid',function(req,res){
    CustomerDao.deleteCustomer(req.params.cid,function(result){
        if(result === "CustomerNotExist"){
            res.status(400).json({result:"Customer Not Exist"});
            return
        }
